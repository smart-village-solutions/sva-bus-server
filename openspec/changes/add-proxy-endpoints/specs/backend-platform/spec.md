...
- **THEN** the service forwards the request to the upstream base URL with the same path, query, and headers (including `api_key` header when configured) and returns the upstream response
